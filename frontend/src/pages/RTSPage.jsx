...
export default function RTSPage() {
  ...
  const fetchRTSConsumers = useCallback(async (query = '') => {
    try {
      setLoading(true);
      const url = query.trim()
        ? `${API_BASE_URL}/consumers/rts?search=${encodeURIComponent(query.trim())}`
        : `${API_BASE_URL}/consumers/rts`;
      const res = await axios.get(url);
      setConsumers(res.data.consumers || []);
    } catch (err) {
      console.error('Error fetching RTS consumers:', err);
      setFeedback({ type: 'error', message: 'Failed to load RTS consumers.' });
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    const handler = setTimeout(() => {
      fetchRTSConsumers(searchTerm);
    }, 300);

    return () => clearTimeout(handler);
  }, [searchTerm, fetchRTSConsumers]);

  const handleStatusUpdate = async (consumerId, newStatus) => {
    const currentConsumer = consumers.find((c) => c.id === consumerId);
    if (currentConsumer?.rts_status === newStatus) return;

    const previousConsumers = [...consumers];
    setConsumers((prev) =>
      prev.map((c) => (c.id === consumerId ? { ...c, rts_status: newStatus } : c))
    );
    setUpdatingId(consumerId);
    setFeedback(null);

    try {
      await axios.patch(`${API_BASE_URL}/consumers/${consumerId}/rts`, {
        rts_status: newStatus,
      });
      setFeedback({
        type: 'success',
        message: `Updated Consumer #${consumerId} (${currentConsumer?.name}) RTS status to "${newStatus}"`,
      });
    } catch (err) {
      console.error('Error updating RTS status:', err);
      setConsumers(previousConsumers);
      setFeedback({
        type: 'error',
        message: err.response?.data?.error || `Failed to update RTS status for #${consumerId}`,
      });
    } finally {
      setUpdatingId(null);
    }
  ...
  const doneCount = consumers.filter((c) => c.rts_status === 'Done').length;
  ...
  const displayedConsumers = consumers.filter((c) => {
    ...
    return c.rts_status === statusFilter;
  });
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-8 bg-gradient-to-br from-emerald-600 via-teal-600 to-slate-900 rounded-3xl p-6 sm:p-8 text-white shadow-lg relative overflow-hidden flex flex-col justify-between">
          <div className="relative z-10 flex items-start justify-between">
            <div className="flex items-center space-x-4">
              <div className="p-3.5 rounded-2xl bg-white/15 backdrop-blur-md border border-white/20">
                <Workflow className="w-7 h-7 text-emerald-200" />
              </div>
              <div>
                <span className="text-[11px] font-bold uppercase tracking-widest text-emerald-200 bg-white/10 px-2.5 py-0.5 rounded-full">
                  Stage 2 &bull; Field &amp; Grid Discom
                </span>
                <h1 className="text-2xl sm:text-3xl font-extrabold mt-1 font-display">
                  Rooftop Solar (RTS) Workflow
                </h1>
                <p className="text-sm text-emerald-100/90 mt-1 max-w-xl">
                  Manage physical site surveys, net-metering approvals, and RTS portal readiness for all registered consumers.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-4 bg-white rounded-3xl p-6 border border-slate-200/90 shadow-sm flex flex-col justify-between">
          ...
          <div className="grid grid-cols-2 gap-3 pt-1">
            <div className="bg-emerald-50/70 border border-emerald-100 rounded-xl px-3.5 py-2">
              <div className="text-[11px] font-semibold text-emerald-700">Completed (Done)</div>
              <div className="text-lg font-extrabold text-emerald-900">{doneCount}</div>
            </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="w-full md:w-96 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-9 py-2.5 rounded-xl border border-slate-200 bg-slate-50/60 focus:bg-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition"
            />
            {searchTerm && (
              <button
      ...
      <div className="bg-white rounded-3xl border border-slate-200/90 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <h2 className="text-xs font-bold text-slate-500 uppercase tracking-wider">
            RTS Consumer Records ({displayedConsumers.length})
          </h2>
          {loading && (
          ...
          <table className="w-full text-left text-sm text-slate-600">
            <thead className="bg-slate-50/80 text-[11px] font-bold uppercase tracking-wider text-slate-400 border-b border-slate-200/70">
              <tr>
                ...
                <th className="px-6 py-4 text-center">RTS Toggle Action</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {displayedConsumers.length === 0 ? (
                ...
                displayedConsumers.map((consumer) => {
                  const isDone = consumer.rts_status === 'Done';
                  const isRowUpdating = updatingId === consumer.id;

                  return (
                    <tr key={consumer.id} className="hover:bg-emerald-50/25 transition">
                      ...
                      <td className="px-6 py-4">
                        {isDone ? (
                          ...
                          <span className="inline-flex items-center space-x-1.5 px-3 py-1 rounded-full text-xs font-bold bg-amber-100 text-amber-800 border border-amber-200/60">
